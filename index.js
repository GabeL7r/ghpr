  ...
  try {
    ...
    if (labels.length > 0) {
      ...
      await client
        ...
        .addLabelsAsync(labels);
    }
  } catch (e) {
    if (e.body.errors[0].message.includes("A pull request already exists")) {
      console.log(
        `A pull request already exists for ${head} -> ${base} in ${owner}/${repoName}`
      );
    } else {
      console.log(
...
function validateInGitRepo() {
  ...
  if (
    shell.exec("git rev-parse --is-inside-work-tree").stdout.trim() !== "true"
  ) {
...
async function renderBody() {
  let body = fs.readFileSync(
    `${getRootLevel()}/.github/pull_request_template.md`,
    "utf8"
  );
